...
const app = express();
const http = require('http'); // Requiring https
...
app.get('/', middlewareSecurity, (request, response) => { 
...
...__dirname
}=*/
// ################################################## SAND BOX ##################################################
...
app.get('/', (request, response) => {
  console.log('Acess on http://localhost:8080')
  let dataObject =  getmovies();
  response.render('pages/index', {dataObject});
})

app.get('/movie/:movieId', (request, response) => {
  console.log('Acess on http://localhost:8080/movie/');
  let dataObject =  getmovies();
  response.render('pages/movie', { dataObject , movieId : request.params.movieId });
})

app.get('/globalmovie/:movieId', (request, response) => {
  let stringData = '';
  let url = `http://www.omdbapi.com/?i=${request.params.movieId}`;
  console.log('Acess on http://localhost:8080/globalmovie/movie/');
  http.get(url,(res)=>{
    res.on('data', chunk =>{
      stringData += chunk;
    })
    res.on('end', res=>{
      ...
      response.render('pages/globalmovie', { dataObject});
...
app.get('/search/:tryToFind', (request, response) => {
  console.log('Acess on http://localhost:8080/search/');
  let dataObject =  getmovies();
  let filteredArray = [];
  for(let i=0;i<dataObject.length;i++){
      if(dataObject[i].title.toLocaleLowerCase().includes(request.params.tryToFind.toLocaleLowerCase())){
        filteredArray.push(dataObject[i]);
    }
  }
  console.log(filteredArray);
  response.render('pages/search', {filteredArray});
})

//############################################## GLOBAL SEARCH ###############################################
app.get('/globalsearch/:tryToFind', (request, response) => {
  console.log('Acess on http://localhost:8080/globalsearch/');
  let dataObject = {};
  let foundArray = []; /// START TO CHANGE
  let stringData = '';
  url =`http://www.omdbapi.com/?s=${request.params.tryToFind}`;
  http.get(url,(res)=>{
    res.on('data', chunk =>{
      stringData += chunk;
    })
    res.on('end', res=>{
      ...
      for(let i=0;i<dataObject.Search.length;i++){
        foundArray.push(dataObject.Search[i]);
        let text = '';
        //########################### More Details About the film ######################
        eachUrl =`http://www.omdbapi.com/?i=${foundArray[i].imdbID}`;
        http.get(eachUrl,(res)=>{
          res.on('data', chunk =>{
            text += chunk;
          });
          res.on('end', res=>{
            dataObject2 = JSON.parse(text);
            filteredArray.push(dataObject2);
              if(filteredArray.length===foundArray.length){
                response.render('pages/globalsearch', {filteredArray});
