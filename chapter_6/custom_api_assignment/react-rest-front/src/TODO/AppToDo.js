...
class AppToDo extends React.Component{
	...
	// 	let str = localStorage.getItem("todoState")
	// 	// console.log(str)
	// 	let obj =  JSON.parse(str);
	// 	this.setState({...obj})

	// Server Method with File Systems
		axios.get("http://localhost:8080/gettodos").then(result => {
	...
	componentDidUpdate() {
		// Local Storage Method
		// localStorage.setItem("todoState",JSON.stringify(this.state))  // Local Storage
		// .then(result => {
    	// 	console.log(result);
		// }).catch(error=>{
    	// 	console.log(error);
		...
		// Server Method with File Systems
		axios.post("http://localhost:8080/newtodos" , this.state);
	}
				...
				{this.props.toDoList.map((toDo,i)=>{
					return(
						<li className={toDo.done&&(this.props.visibility!=='active') ||!toDo.done&&(this.props.visibility!=='complete') ?
							 "list-group-item ":  "visibility list-group-item " } key={toDo.id} >
	...
	render(){	
		return(
			<div>
				...
				<button className="pull-right btn btn-primary btn-size" disabled={this.props.noCompleted} onClick={this.props.clearComplete}>Clear Complete</button>
				<button className="pull-right btn btn-primary btn-size" onClick={this.props.selectAll}>{this.props.noCompleted?"Select All" :"Deselect All"}</button>
			</div>
		)
	}
}
